...
import { StyleSheet, Text, Button, TextInput, SafeAreaView, Dimensions, ScrollView, View, TouchableOpacity } from 'react-native';
...
const App = ({ navigation }) => {
 ...
 var arrx = ["", "Oven", "Coffee Maker", "Toaster", "Electric Water Heater", "Dishwasher", "Washing Machine", "Clothes Dryer", "Monitor"];
  ...
  const Highest1 = () => {
    ...
    for (i = 0; i < newMaxY.length; i++)
    {
      if (newMaxY[i] > newMaxY[high1])
      {
        high1 = i;
      }
    }
  ...
  <Text style={styleSheet.text}>Highest Consuming Appliances</Text>
  

    <Text style={styleSheet.text3}> {flag ? newarrx[Highest1()] : ""} </Text>

    <Text style={styleSheet.text3}> {flag ? newarrx[Highest2()] : ""} </Text>

    <Text style={styleSheet.text3}> {flag ? newarrx[Highest3()] : ""} </Text>
